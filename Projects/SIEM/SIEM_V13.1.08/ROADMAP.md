...
v13.0.00 DONE: the desktop shell baseline. New package src/desktop/ (controller.py Qt-free,
window.py PySide6, main.py entry), requirements-desktop.txt (PySide6 optional, NOT in the core
requirements), launch hook `python launch.py app`. 18 controller tests
(test_desktop_controller_v13.py, no Qt, in the normal suite) + 10 offscreen window smoke tests
(test_desktop_window_v13.py, skips cleanly without PySide6). Full suite 904 green.
* Shell over the existing engine, not a fork | the app launches the SAME src/server/app.py in
  a chosen mode as a subprocess; no SIEM/SOAR logic is duplicated or weakened. Single instance
  (one engine at a time; starting a mode stops the previous). Only spawns the local engine and
  opens a loopback URL, so the anti-C2 posture is intact.
* Logic vs view separated | EngineController holds all decisions and is Qt-free and unit
  tested; window.py is a thin Qt layer (mode buttons, Stop, Open in browser, Create admin,
  Quit, a status line, and a live log pane fed by a QThread reading the engine's stdout).
* No browser for bootstrap | in server mode the Create-admin dialog reuses the exact CLI path
  (core.admin_cli.cmd_create_admin driven non-interactively), preserving the one-admin seal
  invariant. The admin button is enabled only in server mode.
* Honest limitation | the sandbox is headless, so the Qt code is validated by the Qt-free
  controller tests plus an offscreen (QT_QPA_PLATFORM=offscreen) construction smoke test; the
  real visual rendering is validated on a machine with a display.
Remaining for later increments (v13.1+): native panels for the core views (dashboard,
tickets) instead of only launching the web UI, packaging into an installable binary, and the
five-variant SIEM/SOAR mode split below.

v13.1.00 DONE: first native panel. New modules src/desktop/api_client.py (Qt-free) and
src/desktop/panels.py (PySide6). 10 API-client tests (test_desktop_api_v13.py, no Qt, in the
suite) + extended offscreen window smoke. Full suite 917 green.
* Native tickets table | the window now hosts a Log|Tickets tab widget; the Tickets tab is a
  QTableWidget populated from the engine's own /api/tickets over loopback. No logic
  duplicated: the panel reads the same REST API the web UI uses, so the engine stays the
  single source of truth.
* Loopback-only API client | EngineApiClient validates the base URL is loopback and refuses
  anything else (never calls the transport for a non-loopback host), consistent with the
  anti-C2 posture. Every call degrades gracefully: engine down, timeout, non-200, or bad JSON
  returns an empty list, so the panel never crashes.
* Baseline targets no-auth modes (local/showcase); server mode needs login, so the table
  stays empty there until native authentication lands in a later increment.

v13.1.02 DONE: the small AI is available out of the box, and the AI tab is no longer empty.
- the deterministic local classifier now ships enabled by default (SIEM_AI_ENABLED=0 to force
  off); the LLM explainer stays off separately. It is inert until a category is opted in and a
  model exists, so the autonomy ceiling is still the real gate.
- new core/ai/seed.py: a small provenance-tagged (source seed:demo, capped, rollback-able)
  labeled set trains a ticket_triage model and opts the category in. Showcase mode auto-seeds
  on startup; a "Seed AI demo" button (manager+) and POST /api/ai/seed-demo do it on demand,
  then auto-inference populates the AI container from active tickets.
- fixed the AI nav tab rendering "undefined" (missing icon in the ICONS map).

v13.1.03 DONE: our own training data, and our own trained model.
- new core/ai/datasets/soc_cases.py: a curated corpus of 112 labeled tickets across the four
  dispositions, grounded in documented real-world tradecraft (MITRE ATT&CK techniques, known
  tool/malware behaviors, common benign admin patterns). Honest framing: realistic documented
  cases, not data pulled from a production SOC. Grouped into five domain sources
  (windows/linux/email/network/identity) so each stays under the influence cap and is
  rollback-able.
- the seed now trains on this corpus (via the same features the live pipeline extracts) and
  opts ticket_triage in; auto-inference then populates the AI container.
- honest held-out validation (train on one split, score on a disjoint one): ~0.85 accuracy,
  true_positive precision 1.00. Locked in by tests (>= 100 cases, accuracy >= 0.75, TP
  precision >= 0.85, deterministic).

v13.1.04 DONE: cost-sensitive triage biased toward catching threats.
- security priority: a missed threat (false negative) costs far more than a false alarm, so
  the classifier now biases the decision toward the true_positive disposition. NaiveBayes.predict
  gained priority_label/priority_margin (a log-space boost to the priority class); confidence is
  still computed from the UNBIASED scores, so a threat picked only by the bias carries low
  confidence and never clears the auto_close floor (proposed for a human, never auto-closed).
- the bias is a tunable security policy: SIEM_AI_RECALL_BIAS (default 4.0). On the SOC corpus
  it lifts true_positive recall from 0.72 to ~0.94 and cuts missed threats from 5 to 1 on the
  held-out split; pushing it higher trades precision for the last few misses. The metrics
  dashboard reports the biased numbers, since that is what production uses.

v13.1.05 DONE: in-app training and tuning, confidence display, and a better model.
- Training and tuning panel (admin): a "Train on training data" button trains ticket_triage on
  the current corpus and returns a held-out quality estimate; a recall-bias control sets how
  hard the AI leans toward calling a borderline ticket a threat. The bias is persisted
  (ai_settings in the autonomy store) and applied live (AITriage.recall_margin), no longer
  env-only. Routes: GET/POST /api/admin/ai/recall-bias, POST /api/admin/ai/train-datasets.
- Confidence on AI tickets: each shows a colour-coded confidence badge (green >= 0.90, amber
  >= 0.70 "check", red "verify" below), and proposals are sorted lowest-confidence first, so
  analysts spend verification effort where the model is least sure (a 0.60 gets attention a
  0.98 does not).
- Model improvement (feature engineering): extract_ticket_features now adds signal_type x
  risk-factor interaction features and a coarse severity tier. Naive Bayes cannot see joint
  signals on its own, so this sharply improves separation. On the SOC corpus at the default
  bias (now 3.0), held-out true_positive recall reaches 1.00 (zero threats missed) with
  precision ~0.90 and accuracy ~0.94, resolving the earlier recall-vs-precision tradeoff: high
  detection no longer requires a flood of low-confidence false alarms.

v13.1.06 DONE: training documentation, and generalization tested at volume.
- docs/ai_training.md: explains the model, the training data and how to add your own, the
  features, the training loop, the cost-sensitive recall bias, and the evaluation, written to
  be honest about limits.
- src/core/ai/datasets/synth.py: a seedable synthetic case generator with a ground-truth
  oracle and deliberately hard cases (low-severity real attacks, look-alike benign, novel
  tokens), for generalization testing on fresh, larger volumes.
- Finding: the held-out 1.00 recall on the ~33-case split was small-sample optimism. On 1000
  fresh cases the default bias of 3.0 missed ~3% of threats; 5.0 caught essentially all (0 to
  1 per thousand) with a modest false-alarm rate. The default bias is now 5.0, chosen from the
  volume test, not the small split. Overall accuracy ~0.73 to 0.75 reflects the intended
  recall-over-precision trade. test_ai_generalization_v13.py locks in recall >= 0.98 at volume.

v13.1.07 DONE: the confidence safety net (no high-confidence misses).
- Measured failure: at volume, a threat could be dismissed with HIGH confidence (0.927), which
  is the one dangerous failure because a confident disposition is never re-checked.
- Fix: src/core/ai/threat_indicators.py, a deterministic backstop grounded in real ATT&CK
  behaviors. If a ticket carries a recognized malicious indicator (behavioral risk factor or an
  unambiguously offensive technique) but the model dismisses it as a non-threat, the reported
  confidence is capped at 0.65 (below the review threshold). The label is unchanged; only
  confidence is lowered, so the item lands in the analyst verify queue. Benign activity carries
  no indicator, so genuine high-confidence dispositions are untouched.
- Result: across 7000 fresh cases, ZERO missed threats above 0.70 confidence (every miss <=
  0.65, therefore reviewed). The tool's safety no longer depends on the model never missing,
  only on a miss never being confident. test_ai_threat_indicators_v13.py and the rewritten
  generalization test lock this in.
- Margin default kept at 5.0: with the net, 3.0 and 5.0 are both safe (zero high-confidence
  misses); 5.0 catches essentially every threat directly (~1 miss/1000) rather than leaving
  ~67 low-confidence misses per 1000 to the review queue, at ~0.018 accuracy cost. Tunable.

### v13.2 | keyboard shortcuts and power-user editing (PLANNED)
Requested: a customizable keyboard-shortcut system directly in the app, PowerPoint/IDE style.
Not just fixed hotkeys, the user can view, create, edit and reassign shortcuts, and there is a
panel to see and manage them all.
* Shortcut registry | a central table of named ACTIONS (find, select-all, multi-select
  extend, expand/collapse selected rows, open/close ticket, next/previous, refresh, focus
  filter, etc.) each mapped to a key binding. Defaults ship built-in; user overrides are
  persisted (server-side per account so they follow the user, mirroring the profile design),
  with a conflict detector that refuses two actions bound to the same chord.
* Built-in defaults | Ctrl+F find/filter, Ctrl+A select all, Ctrl/Shift+click and
  Shift+Arrow to extend a multi-selection, Enter/Space to open, a collapse/expand chord for
  selected rows, arrow-key navigation, Esc to clear selection. A searchable shortcuts panel
  (cheat sheet) lists them and is where the user edits or adds their own.
* Multi-select + bulk actions | select several tickets/rows and act on all at once (open,
  close, assign, delegate to AI, set disposition), the same bulk operations the mouse offers,
  from the keyboard.
* Two surfaces, one model | in the desktop app this maps to QAction + QKeySequence/QShortcut;
  in the web UI to a keydown dispatcher reading the same registry. The action set and the
  user's custom bindings are shared so behavior matches across both. Security note: a shortcut
  only triggers an action the user is already authorized for; it is a convenience layer over
  existing authz, never a bypass. Bulk actions that hit sensitive operations still go through
  the same dual-control/RBAC gates as the single-item path.
...
v13.1.07 DONE: confidence safety net (no high-confidence misses) + minimal-margin default.
- The dangerous failure is a threat dismissed with HIGH confidence (never re-checked), not a
  threat dismissed at low confidence (the review queue catches those). New module
  src/core/ai/threat_indicators.py: if a ticket carries a recognized malicious indicator
  (behavioral risk factor or unambiguously offensive ATT&CK technique) but the model dismisses
  it, classify_ticket caps the reported confidence at 0.65 (below the review threshold). Label
  unchanged; only confidence lowered, so the item is surfaced for human verification.
- Measured guarantee: on 7000 fresh synthetic cases, ZERO missed threats exceed 0.70 confidence
  (every miss <= 0.65). Locked in by test_ai_generalization_v13.py and test_ai_threat_indicators_v13.py.
- Because misses are now guaranteed low-confidence and therefore reviewed, the default recall
  bias is lowered to the MINIMAL value that gives the cleanest accuracy (3.0), rather than the
  value chasing perfect raw recall. This is the "minimal margin, cleanest accuracy, no threat
  slips past unseen" operating point.
