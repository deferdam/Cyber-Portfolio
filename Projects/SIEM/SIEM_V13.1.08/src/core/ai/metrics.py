...
def confusion(examples: List[Tuple[List[str], str]], model: NaiveBayes,
              priority_label: str = None, priority_margin: float = 0.0) -> Dict:
    ...
    for feats, true_label in examples:
        pred = model.predict(feats, priority_label=priority_label,
                             priority_margin=priority_margin).label
...
def per_class_metrics(examples: List[Tuple[List[str], str]], model: NaiveBayes,
                      priority_label: str = None, priority_margin: float = 0.0) -> Dict:
...
def evaluate_holdout(examples: List[Tuple[List[str], str]],
    ...
    conf = confusion(test, model, priority_label, priority_margin)
    return {
        "enough_data": True,
        ...
        "per_class": per_class_metrics(test, model, priority_label, priority_margin),
    }
