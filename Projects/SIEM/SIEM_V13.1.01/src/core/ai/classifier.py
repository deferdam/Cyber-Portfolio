...
class NaiveBayes:
    ...
    def predict(self, feats: List[str], version: int = 0, abstain_below: float = 0.0) -> Prediction:
        if not self.trained:
            return Prediction("unknown", 0.0, [], version, abstained=True)
        scores = self._log_prob(sorted(set(feats)))
        # softmax over log-scores for a bounded confidence, deterministic tie-break by label
        top = max(sorted(scores), key=lambda c: (scores[c], c))
