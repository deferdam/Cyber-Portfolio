...
from detect.modules.ai.ai_baseline import load_default, match_framework, is_known_ai_port, observe
        ...
        if _is_bind(ev):
            if fw is None:
                ...
                file_hashes=extract_hashes(ev),
                    ...
                    recommended_actions=["Verify the process binary.", "Compare the hash against the legitimate binary.", "Isolate the AI service."],
        ...
        if _is_connect(ev):
            if fw is None:
                ...
                file_hashes=extract_hashes(ev),
                    ...
                    recommended_actions=["Check the client configuration (endpoint, port).", "Check for an unauthorized local proxy."],
