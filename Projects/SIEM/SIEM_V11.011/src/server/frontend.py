...
const TABS=[['dash','Dashboard'],['open','Open'],['mine','My Tickets'],['signals','Signals'],['run','Run SIEM'],['templates','Templates'],['admin','Admin']];
let _isAdmin=false;
let SHOWCASE=false;
const VERSION='v11.011';   // single source; bumped on every new build/zip
...
async function renderAdmin(el){
  let status={},accounts=[],pending=[],audit=[];
  ...
  el.innerHTML=
    ...
    '<div class="card"><h3>Accounts</h3>'+
      '<div class="admin-newacct">'+
        ...
        '<input id="na-pass" type="password" class="login-input" placeholder="Password" style="margin-bottom:8px">'+
...
async function init(){  // restore theme
  ...
  // Bootstrap gate: if this is a virgin install (no admin yet), show the one-time setup
  // screen instead of the login screen. Runs BEFORE the login gate, since an account
  // that does not exist yet cannot be logged into. /api/setup/status 404s once an admin
  // exists, so this naturally stops mattering after the first real setup.
  try{
  ...
  if(c.authenticated){_isAdmin=(c.role==='admin');showLogoutControl(c.user,c.role);}
  renderFooter();
  checkUpdateStatus();
  buildNav();
...
function showSetup(err){
  ...
  host.innerHTML=
    '<div class="login-wrap"><div class="login-card">'+
      ...
      '<input id="su-token" class="login-input" placeholder="Setup token" autocomplete="off">'+
      '<input id="su-user" class="login-input" placeholder="Admin username" autocomplete="username">'+
      '<input id="su-pass" type="password" class="login-input" placeholder="Password" autocomplete="new-password">'+
...
async function doSetup(){
  ...
  if(pass!==pass2){showSetup('Passwords do not match.');return;}
  ...
  }catch(e){showSetup('Network error. Try again.');return;}
  if(!r.ok){
    let data={};try{data=await r.json();}catch(e){}
    showSetup(data.error||'Setup failed. Check the token and try again.');
    return;
  }
