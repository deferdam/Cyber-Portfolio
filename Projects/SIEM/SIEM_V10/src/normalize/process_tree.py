...
class ProcessTree:
    ...
    # -- Enriched node queries (read-only, for signal context) -----------------

    @staticmethod
    def _node_dict(n: "ProcessNode") -> dict:
        return {
            "image": n.image,
            "pid": n.pid,
            "ppid": n.ppid,
    ...
    def ancestor_nodes(self, host: str, pid: int, max_depth: int = 20) -> List[dict]:
        """Return the enriched ancestor chain for a process (root first).

        Walks ppid links scoped to the host. Cycle guard via visited set
        and max_depth. Pure read, no mutation.
        """
        chain: List[dict] = []
        node = self._nodes.get((host, pid))
        visited: set = set()
        depth = 0
        while node and node.ppid and depth < max_depth:
            if node.pid in visited:
                break
            visited.add(node.pid)
            parent = self._nodes.get((host, node.ppid))
            if not parent:
                break
            chain.insert(0, self._node_dict(parent))  # root ends up first
            node = parent
            depth += 1
        return chain

    def child_nodes(self, host: str, pid: int) -> List[dict]:
        """Return enriched direct children of a process instance (host, pid)."""
        return [
            self._node_dict(n)
            for n in self._nodes.values()
            if n.host == host and n.ppid == pid
        ]
