...
let _role='operator';
let SHOWCASE=false;
const VERSION='v13.1.01';   // single source; bumped on every new build/zip
...
async function renderAI(el){
  ...
  function row(rec){
    ...
    return '<div class="wk-row" style="flex-wrap:wrap;gap:8px;align-items:center">'+
      '<div class="wk-info" style="min-width:150px"><span class="wk-name">'+esc(rec.ticket_id)+'</span>'+
        '<span class="wk-tag">AI: '+lab+'</span><span class="wk-tag">conf '+conf+'</span>'+
  ...
  const propRows=proposed.map(row).join('');
