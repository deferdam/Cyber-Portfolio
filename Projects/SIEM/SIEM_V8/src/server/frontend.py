"""frontend.py - Embedded SPA for the Mini SOAR web interface (v8 UI tranche A.2)."""
...
<div class="topbar">
  ...
  <div class="brand">Mini SOAR <span>v8</span></div>
...
<div class="drawer" id="drawer">
  ...
  <div class="dr-section">
    ...
    <div class="dr-muted">No shared contacts in local mode. Multi user roster arrives in v10 (server mode).</div>
  </div>

  <div class="dr-note">Mode is fixed at launch. Restart with run_server.bat to switch.</div>
</div>
...
const FORMATS=[
  ...
  {v:'auditd',l:'auditd (v9)',enabled:false},
  {v:'evtx',l:'Windows EVTX (v9)',enabled:false},
  {v:'pcap',l:'PCAP / Wireshark (v9)',enabled:false},
  {v:'elastic',l:'Elastic / ECS (v9)',enabled:false},
  {v:'snort',l:'Snort alerts (v9)',enabled:false},
  {v:'csv',l:'Generic CSV (v9)',enabled:false}
];
...
function renderRun(el){
  ...
  '<div class="run-wrap"><div class="card"><h3>Pick an input file</h3>'+
    ...
    '</div>'+
    '<div class="fmt-note">Formats marked (v9) will be available in the next version.</div>'+
  '</div>'+
...
function renderTemplates(el){
  ...
  '<div class="card" style="margin-bottom:16px"><h3>Note templates</h3>'+
    '<div style="font-size:12px;color:var(--muted);line-height:1.6">One template per ticket status. The Insert template button in a ticket fills the template that matches its current status and substitutes the variables below. Edits are saved on this machine. In v10 this editing will be limited to roles such as manager or CISO, to keep one writing convention.</div>'+
