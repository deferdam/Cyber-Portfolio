        ...
        for fpath in paths:
            ...
            if matched:
                sigs.append(_make("auditd.sensitive_file_access", ev, 0.80, 0.78,
                    ...
                    ["Verify the process accessing the file."],
    ...
    for ev in events:
        ...
        if triggered:
            sigs.append(_make("auditd.setuid_chmod", ev, 0.85, 0.82,
                ...
                ["Identify the affected binary."], "Privilege Escalation", "T1548.001"))
    return sigs
        ...
        for pat in _USERADD:
            if pat.search(cl):
                ...
                sigs.append(_make("auditd.user_creation", ev, score, score-0.05,
                    ...
                    ["Verify whether this is authorized."], "Persistence", "T1136.001"))
                break
    return sigs
...
def _suspicious_connect(events):
    ...
    for ev in events:
        ...
        sigs.append(_make("auditd.suspicious_connect", ev, 0.88, 0.80,
            ...
            ["Block the destination IP."], "Command and Control", "T1071.001"))
    return sigs
        ...
        for pat, score, label, tactic, tech in _EXECVE_PATS:
            if pat.search(cl):
                sigs.append(_make("auditd.execve_suspicious", ev, score, score-0.05,
                    ...
                    ["Verify the context."], tactic, tech))
                break
    return sigs
