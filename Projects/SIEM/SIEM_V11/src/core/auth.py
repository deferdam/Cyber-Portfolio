...
def current_principal(request=None, mode="local") -> Principal:
    """Resolve who is making this request. In local mode with no login configured, this
    stays the local operator (single-user convenience). The server wires real session
    resolution in app.py before_request, calling principal_from_session when a valid
    cookie is present. This remains the seam's default fallback."""
