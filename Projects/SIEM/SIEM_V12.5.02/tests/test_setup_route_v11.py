...
os.environ["SIEM_REQUIRE_LOGIN"] = "1"  # setup/bootstrap only applies when login is required
...
# reusing the one-time setup token AS the password is refused, token NOT burned
r = c.post("/api/setup", json={"token":TOKEN,"username":"root","password":TOKEN})
check("token-as-password -> 400", r.status_code==400)
check("400 body names the token reuse", "token" in (r.get_json() or {}).get("error","").lower())
check("token still active after token-as-pw attempt", appmod._bootstrap_token.is_active())
