...
v10 will add a second universal launcher to choose LOCAL vs MULTI (server) once the server
skeleton exists.
"""
import atexit
import os
...
APP_MODES = {"local": "local", "showcase": "showcase"}
...
MENU = [
    ...
    ("server",   "Server / Multi",  "Multi-user mode. Not implemented yet (v10).",         "disabled"),
]
...
def main():
    if len(sys.argv) > 1:
        sys.exit(run_target(sys.argv[1], extra=sys.argv[2:]))
    serve_ui()
