...
.login-wrap{position:fixed;inset:0;display:flex;align-items:center;justify-content:center;background:var(--bg,#15171a);}
.login-card{width:320px;background:var(--s1,#1d2024);border:1px solid var(--border,#2a2e33);border-radius:14px;padding:28px 26px;box-shadow:0 8px 40px rgba(0,0,0,.4);}
.login-title{font-size:22px;font-weight:700;color:var(--text,#e6e8ea);margin-bottom:2px;}
...
.login-input{width:100%;box-sizing:border-box;background:var(--s2,#24272c);border:1px solid var(--border,#2a2e33);border-radius:9px;padding:11px 12px;color:var(--text,#e6e8ea);font-size:14px;margin-bottom:12px;}
.login-input:focus{outline:none;border-color:var(--green,#3fae74);}
.login-btn{width:100%;background:var(--green,#3fae74);color:#0a0c0d;border:none;border-radius:9px;padding:11px;font-weight:700;font-size:14px;cursor:pointer;}
...
.mfa-secret{font-family:Consolas,monospace;font-size:15px;letter-spacing:2px;color:var(--text,#e6e8ea);background:var(--s2,#24272c);padding:10px;border-radius:8px;margin:6px 0 10px;text-align:center;user-select:all;}
...
let SHOWCASE=false;
const VERSION='v11.002';   // single source; bumped on every new build/zip
...
function showServerBanner(){
  ...
  b.textContent='SERVER MODE (v10 skeleton) | no accounts or authentication yet, read-only host posture. Real multi-user accounts arrive in v11.';
...
async function init(){  // restore theme
  ...
  // Login gate: if the server requires login and we are not authenticated, show the
  // login screen and stop here. The rest of the app boots only after a successful login.
  if(c.require_login && !c.authenticated){
    showLogin();
    return;
  }
  if(c.authenticated){showLogoutControl(c.user,c.role);}
  buildNav();
...
function showLogin(err,mfa,user,pass){
  ...
  host.innerHTML=
    '<div class="login-wrap"><div class="login-card">'+
      ...
      '<button id="lg-btn" class="login-btn" onclick="doLogin()">'+(mfa?'Verify':'Sign in')+'</button>'+
    '</div></div>';
...
async function openMfaPanel(){
  ...
  let inner;
  if(st.enabled){
    inner='<div class="mfa-state on">Two-factor is ENABLED on your account.</div>'+
      ...
      '<button class="login-btn" style="background:#b5524f;color:#fff" onclick="mfaDisable()">Disable two-factor</button>';
  }else{
    inner='<div class="mfa-state off">Two-factor is OFF.</div>'+
      '<p class="mfa-help">Add a second factor with any authenticator app (Google Authenticator, Aegis, FreeOTP).</p>'+
      '<button class="login-btn" onclick="mfaBeginEnroll()">Set up two-factor</button>';
  }
