...
# system-prompt actions were deliberately left out before the AI module existed; now that
# core/ai/ exists (v12), the three AI actions that expand the AI's real-world authority are
# listed: raising a category's autonomy ceiling to auto-close (final disposition), turning
# the AI's global kill switch back off (restores autonomy), and retraining a model (changes
# what the AI will do next). Actions that only ever REMOVE authority (lowering a ceiling,
# engaging the kill switch) are not listed: they are safety actions and always apply
# immediately, exactly like force_logout.
SENSITIVE_ACTIONS = (
    "create_account",
    "change_role",
    "delete_account",
    ...
    "set_ai_ceiling_auto_close",
    "disengage_ai_kill_switch",
    "train_ai_model",
)
...
class AccountStore:
    ...
    def _migrate(self) -> None:
        ...
        try:
            ...
            for pcol in ("first_name", "last_name", "email"):
                if pcol not in cols:
                    conn.execute("ALTER TABLE accounts ADD COLUMN %s TEXT" % pcol)
    ...
    # Cosmetic profile metadata (display name, email). NOT security-relevant like role or
    # password, so it is not dual-control gated; the route enforces self-or-admin authz.
    PROFILE_FIELD_MAX = 120

    def set_profile(self, username: str, first_name: str = "", last_name: str = "",
                    email: str = "") -> None:
        ...
        def _clean(v):
            v = (v or "").strip()
            if len(v) > self.PROFILE_FIELD_MAX:
                raise AccountError("Profile field too long (max %d)" % self.PROFILE_FIELD_MAX)
            return v
        f, l, e = _clean(first_name), _clean(last_name), _clean(email)
        conn = self._connect()
        try:
            conn.execute(
                "UPDATE accounts SET first_name=?, last_name=?, email=? WHERE username=?",
                (f, l, e, username))
            conn.commit()
        finally:
            conn.close()
    ...
    def get_profile(self, username: str) -> Dict:
        ...
        return {"username": username,
                "first_name": acct.get("first_name") or "",
                "last_name": acct.get("last_name") or "",
