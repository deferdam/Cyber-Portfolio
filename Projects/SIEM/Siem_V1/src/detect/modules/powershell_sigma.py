...
def _severity_from_level(level: str) -> str:
    """Convert Sigma level string to internal severity vocabulary."""
    lv = (level or "").lower()
    if lv == "critical":
        return "critical"
    if lv == "high":
...
def correlate_recon_sequence(
    ...
    Design notes:
      - emitted_pairs prevents duplicate correlation signals in O(n²) loop
...
def run(events: List[CanonicalEvent], rule_path: str = "powershell_suspicious.yaml") -> List[Signal]:
    """
    Match PowerShell 4104 events against the Sigma rule at rule_path.

    Returns a list of powershell_sigma Signals.
    Does NOT run temporal correlation — call correlate_recon_sequence() separately.

    Scoring V1:
      base 0.6 + 0.1 per matched selection, capped at 1.0
      score == confidence by design (V1 simplification)
    """
    path = Path(rule_path)
    if not path.exists():
        return []
    ...
    for ev in events:
        ...
        for sel_name, (field, needles) in rule.selections.items():
            ...
            # Sigma OR semantics: ANY needle in the list fires this selection
        ...
        signals.append(
            Signal(
                ...
                recommended_actions=[
                    "Inspect ScriptBlockText and decode payload if EncodedCommand is present.",
                    "Correlate with parent process and network/file activity within ±10 minutes.",
                    "Validate whether this is legitimate admin automation (known script, known host).",
                ],
            )
        )
