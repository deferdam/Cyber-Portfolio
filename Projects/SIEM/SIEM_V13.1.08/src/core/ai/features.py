...
def extract_ticket_features(ticket: Dict) -> List[str]:
    ...
    for rf in (t.get("risk_factors") or []):
        ...
        if tok:
            ...
            # Interaction feature: a risk factor means very different things depending on the
            # signal type (encoded_command on powershell vs on a benign admin script). Naive
            # Bayes treats features independently, so an explicit signal_type x risk pairing
            # gives it the joint signal it otherwise cannot see. This markedly sharpens the
            # separation between real threats and look-alike benign activity.
            if stype:
                feats.append("sr=%s|%s" % (stype.replace(" ", "_"), tok))

    # Coarse severity tier: a single strong threat-vs-noise signal that generalizes better
    # than the exact severity string.
    if sev:
        feats.append("sevtier=%s" % ("hi" if sev in ("critical", "high") else "lo"))
